...
const DonHuy = () => {
  ...
  const getAllOrder = async () => {
    let arr = [];
    await axios
      .get(`${GET_ALL_USER_ORDERS}?id=${info.id}`)
      .then((res) => {
      ...
      Alert.alert(
        ...
        [
          ...
          },
          {
            ...
            onPress: async () => {
              await axios
                .put(HUY_USER_ORDERS, data)
                .then((res) => {
                  if (res.data.errCode == 0) {
                    Alert.alert(
                      "Thông báo",
                      "Đơn hàng đã được hủy, bạn hãy chờ bên Shop duyệt hủy đơn hàng",
                      [
                        {
                          text: "OK",
                          onPress: () => {
          ...
          >
            ...
            <View style={{}}>
              ...
              >
                ...
                <Text>Bộ nhớ {getSize(IdSP, item.id)}</Text>
      ...
      {listDonHang.map((item, index) => {
        return (
          <Pressable
            ...
            onLongPress={() => {
              item.status == 5 && handleShowDelete(item.id);
            }}
            key={item.id}
            style={{
              backgroundColor: "#fff",
          ...
          >
            ...
            >
              <Text
                style={{
                  fontWeight: "600",
                  color: item.status === 4 ? "#FF0000 " : "#008000",
                }}
              >
                {item.status === 4
                  ? "Đang xác nhận hủy đơn"
                  : `Đã hủy đơn, tiền của bạn đã được hoàn lại:  +${price(
                      item.tongTien
                    )} `}{" "}
              </Text>
            </View>
            ...
            >
              <Text style={{ fontWeight: "600" }}>
                Số Sản phẩm: {tongSoSanPham(item.idCart)}
              </Text>
              <Text
                style={{
                  fontSize: 18,
                  fontWeight: "700",
                  textDecorationLine:
                    item.status === 5 ? "line-through" : "none",
                }}
              >
                Tổng:{" "}
