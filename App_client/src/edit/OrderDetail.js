...
const OrderDetail = (props) => {
  ...
  const getAllOrder = async () => {
    let arr = [];
    await axios
      .get(`${GET_ALL_USER_ORDERS}?id=${idUser}`)
      .then((res) => {
      ...
      return (
        <View key={index}>
          ...
          >
            ...
            />
            <View style={{}}>
              <Text style={{ width: "80%", padding: 6, alignItems: "center" }}>
                {item.tenSp}
              </Text>
              <View
                style={{
              ...
              >
                {item.sale <= 0 ? (
                  ...
                  <View style={{ flexDirection: "row", marginLeft: 4 }}>
                    ...
                    >
                      {price(
                        item.giaSanPham - item.giaSanPham * (item.sale / 100)
                      )}
                    </Text>
                  </View>
                )}
      ...
      <View style={{ marginBottom: 10, marginTop: 5 }}>
        ...
        <View style={styles.infoUser}>
          <Text style={styles.textInfoUser}>
            Họ và tên: <Text style={styles.textName}>{info.tenThanhVien}</Text>
          </Text>
          <Text style={styles.textInfoUser}>
            Số Điện thoại:{" "}
            <Text style={styles.textName}>{info.soDienThoai}</Text>
          </Text>
          <Text style={styles.textInfoUser}>
            Địa chỉ nhận hàng:{" "}
            <Text style={styles.textName}>{info.diaChi}</Text>
          </Text>
          <Text style={styles.textInfoUser}>
            Ngày đặt đơn:{" "}
            <Text style={styles.textName}>
              {(getOrder && getOrder.status == 0) || status == 1
                ? formatDate(getOrder.createdAt)
                : formatDate(getOrder.updatedAt)}
            </Text>
          </Text>
          <Text style={styles.textInfoUser}>
            Tổng tiền:{" "}
            <Text style={styles.textName}>
              {price(getOrder.tongTien ? getOrder.tongTien : "")}
            </Text>
          </Text>
          <Text style={styles.textInfoUser}>
            Trạng thái đơn hàng:{" "}
            ...
            >
              {getOrder && getOrder.status == 0
                ? "Đang chờ xử duyệt đơn"
                : getOrder.status == 1
                ? "Đã xác nhận đơn hàng"
                : getOrder.status == 2
                ? "Đơn của bạn đang được giao "
                : getOrder.status == 3
                ? "Giao thành công"
                : getOrder.status == 4
        ...
        <ScrollView style={{ maxHeight: 400 }}>
          ...
          >
            <Text style={{ fontWeight: "600" }}>
              Số Sản phẩm: {tongSoSanPham(idCart)}
            </Text>
            <Text style={{ fontSize: 18, fontWeight: "700" }}>
              Tổng:{" "}
              <Text style={{ fontSize: 17, color: "#B22222" }}>
                {price(tongTien ? tongTien : "")}
              </Text>{" "}
            </Text>
          </View>
        </ScrollView>
